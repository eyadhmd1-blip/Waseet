// ============================================================
// WASEET — notify-urgent Edge Function
// Called after an urgent request is created.
// Finds available providers in the same city+category and
// sends them a high-priority Expo push notification.
// ============================================================

import { serve } from 'https://deno.land/std@0.177.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';

const EXPO_PUSH_URL = 'https://exp.host/--/api/v2/push/send';
const BATCH_SIZE    = 50;

serve(async (req) => {
  try {
    const { request_id, city, category_slug } = await req.json();
    if (!request_id || !city || !category_slug) {
      return new Response(JSON.stringify({ error: 'missing params' }), { status: 400 });
    }

    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    );

    // Fetch request title for notification copy
    const { data: req_row } = await supabase
      .from('requests')
      .select('title, city')
      .eq('id', request_id)
      .single();

    // Fetch available provider tokens via RPC
    const { data: targets } = await supabase.rpc('get_available_providers_for_urgent', {
      p_city: city,
      p_category_slug: category_slug,
    });

    if (!targets || targets.length === 0) {
      return new Response(JSON.stringify({ sent: 0, message: 'no available providers' }), { status: 200 });
    }

    // Build Expo messages
    const messages = targets.map((t: { token: string }) => ({
      to:    t.token,
      title: '🚨 طلب طارئ!',
      body:  `${req_row?.title ?? 'طلب طارئ'} — ${city} · +25% عمولة`,
      data:  { screen: 'urgent', request_id, notif_id: request_id },
      sound: 'default',
      priority: 'high',
      channelId: 'urgent',
    }));

    // Send in batches
    let sent = 0;
    for (let i = 0; i < messages.length; i += BATCH_SIZE) {
      const batch = messages.slice(i, i + BATCH_SIZE);
      const res = await fetch(EXPO_PUSH_URL, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body:    JSON.stringify(batch),
      });
      if (res.ok) sent += batch.length;
    }

    return new Response(JSON.stringify({ sent }), { status: 200 });

  } catch (err) {
    return new Response(JSON.stringify({ error: String(err) }), { status: 500 });
  }
});
