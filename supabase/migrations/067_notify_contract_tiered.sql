-- ============================================================
-- Migration 067: upgrade get_available_providers_for_contract
-- Applies the same 3-tier notification logic used for normal
-- requests (migration 066) to recurring contract notifications.
--
...
-- Tier 3 — medium-inactive (31–90 days)
--           → max 1 notification per 24h (type='new_contract')
-- Tier 4 (>90 days) → excluded entirely
-- ============================================================

-- ── Index for tier-3 cooldown (new_contract type) ────────────
CREATE INDEX IF NOT EXISTS idx_notifications_new_contract_cooldown
...
AS $$
  ...
  WHERE u.city            = p_city
    ...
    AND (
      ...
      OR
      -- Tier 3: lapsed 31–90 days — max 1 notification per 24h
      (
        pr.is_subscribed    = FALSE
        AND pr.subscription_ends IS NOT NULL
  ...
  ORDER BY pr.id, pt.updated_at DESC
  LIMIT 300;
$$;

GRANT EXECUTE ON FUNCTION get_available_providers_for_contract(TEXT, TEXT) TO service_role;
